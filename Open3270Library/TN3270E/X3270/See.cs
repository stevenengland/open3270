...
namespace StEn.Open3270.TN3270E.X3270
{
    internal class See
    {
        ...
        // WCC definitions
        public static bool WCC_RESET(byte c)
        {
            return (c & 0x40) != 0;
        }

        public static bool WCC_START_PRINTER(byte c)
        {
            return (c & 0x08) != 0;
        }

        public static bool WCC_SOUND_ALARM(byte c)
        {
            return (c & 0x04) != 0;
        }

        public static bool WCC_KEYBOARD_RESTORE(byte c)
        {
            return (c & 0x02) != 0;
        }

        public static bool WCC_RESET_MDT(byte c)
        {
            return (c & 0x01) != 0;
        }


        public static string FormatUnknown(byte vvalue)
        {
            return "unknown[= 0x" + vvalue + "]";
        }

        public static string GetEbc(byte ch)
        {
            switch (ch)
            {
                case FCORDER_NULL:
            ...
            if (Tables.Ebc2Ascii[ch] != 0)
                return "" + Convert.ToChar(Tables.Ebc2Ascii[ch]);
            return "" + Convert.ToChar(ch);
        }

        public static string GetAidFromCode(byte code)
        {
            switch (code)
            {
                case AID_NO:
        ...
        public static string GetSeeAttribute(byte fa)
        {
            var seeAttributeBuffer = "";
            var paren = "(";
        ...
        public static string GetHighlight(byte setting)
        {
            switch (setting)
            {
                case XAH_DEFAULT:
        ...
        public static string GetColor(byte setting)
        {
            string[] colorName =
            {
                "neutralBlack",
        ...
        public static string GetTransparency(byte setting)
        {
            switch (setting)
            {
                case XAT_DEFAULT:
        ...
        public static string GetValidation(byte setting)
        {
            var see_validation_buf = "";
            var paren = "(";
        ...
        public static string GetOutline(byte setting)
        {
            var seeOutlineBuffer = "";
            var paren = "(";
        ...
        public static string GetEfa(byte efa, byte value)
        {
            switch (efa)
            {
                case XA_ALL:
        ...
        public static string GetEfaUnformatted(byte efa, byte vvalue)
        {
            switch (efa)
            {
                case XA_ALL:
        ...
        public static string GetEfaOnly(byte efa)
        {
            switch (efa)
            {
                case XA_ALL:
        ...
        public static string GetQCodeode(byte id)
        {
            switch (id)
            {
                case QR_CHARSETS:
