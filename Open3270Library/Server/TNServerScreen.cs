...
namespace StEn.Open3270.Server
{
    ...
    public class TNServerScreen
    {
        ...
        private static readonly byte[] inboundAddrChars =
        {
        ...
        private Hashtable hashAidToText;

        //
        private Hashtable hashTextToAid;
        public bool inExtendedMode = false;


        public byte lastAid;
        public byte[] mScreenBytes;
        private readonly ArrayList mStrings;
            ...
            // PCH pchFrom, PCH pchTo, BOOL fScreen, int rows, int columns)
        {
            ...
            while (fromIndex < from.Length && from[fromIndex] != 0)
            {
                if (fScreen)
                {
                    ...
                    {
                        case ']':
                            to[toIndex++] = ATTR_NORM;
                            break;
                        case '}':
                            to[toIndex++] = ATTR_BOLD;
                            break;
                        case '[':
                            to[toIndex++] = ATTR_INP;
                            break;
                        case '{':
                            to[toIndex++] = ATTR_INP_BOLD;
                            break;
                        case '~':
                            to[toIndex++] = ATTR_HIDDEN;
                            break;
                        case '^':
                            to[toIndex++] = ATTR_PASSWORD;
                            break;
                        default:
                ...
                else
                {
                    ...
                    {
                        case ']':
                            to[toIndex++] = ATTR_3270_NORM;
                            break;
                        case '}':
                            to[toIndex++] = ATTR_3270_BOLD;
                            break;
                        case '[':
                            to[toIndex++] = ATTR_3270_INPUT;
                            break;
                        case '{':
                            to[toIndex++] = ATTR_3270_INPUT_BOLD;
                            break;
                        case '~':
                            to[toIndex++] = ATTR_3270_HIDDEN;
                            break;
                        case '^':
                            to[toIndex++] = ATTR_3270_PASSWORD;
                            break;
                        default:
                            to[toIndex++] = Tables.A2E[(byte) from[fromIndex]];
                            break;
        ...
        public byte TextToAid(string pszText)
        {
            ...
            var aidKey = hashTextToAid[pszText.ToLower()] as string;
            if (aidKey == null)
                return 0;
        ...
        public string ReadScreen(int position, int end)
        {
            ...
            {
                text += Convert.ToChar(Tables.E2A[mScreenBytes[i]]);
            }
            return text.TrimEnd();
        }
